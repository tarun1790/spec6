"use strict";(self.webpackChunk_N_E=self.webpackChunk_N_E||[]).push([[7486],{10965:function(e,t,a){function r(e,t){e.accDescr&&t.setAccDescription?.(e.accDescr),e.accTitle&&t.setAccTitle?.(e.accTitle),e.title&&t.setDiagramTitle?.(e.title)}a.d(t,{A:function(){return r}}),(0,a(51131).e)(r,"populateCommonDb")},7486:function(e,t,a){a.d(t,{diagram:function(){return I}});var r=a(10965),i=a(25618),n=a(85599),l=a(1198),s=a(90498),o=a(51131),c=a(54327),d={showLegend:!0,ticks:5,max:null,min:0,graticule:"circle"},u={axes:[],curves:[],options:d},g=structuredClone(u),p=l.vZ.radar,x=(0,o.e)(()=>(0,n.Rb)({...p,...(0,l.iE)().radar}),"getConfig"),h=(0,o.e)(()=>g.axes,"getAxes"),m=(0,o.e)(()=>g.curves,"getCurves"),f=(0,o.e)(()=>g.options,"getOptions"),$=(0,o.e)(e=>{g.axes=e.map(e=>({name:e.name,label:e.label??e.name}))},"setAxes"),y=(0,o.e)(e=>{g.curves=e.map(e=>({name:e.name,label:e.label??e.name,entries:v(e.entries)}))},"setCurves"),v=(0,o.e)(e=>{if(void 0==e[0].axis)return e.map(e=>e.value);let t=h();if(0===t.length)throw Error("Axes must be populated before curves for reference entries");return t.map(t=>{let a=e.find(e=>e.axis?.$refText===t.name);if(void 0===a)throw Error("Missing entry for axis "+t.label);return a.value})},"computeCurveEntries"),w={getAxes:h,getCurves:m,getOptions:f,setAxes:$,setCurves:y,setOptions:(0,o.e)(e=>{let t=e.reduce((e,t)=>(e[t.name]=t,e),{});g.options={showLegend:t.showLegend?.value??d.showLegend,ticks:t.ticks?.value??d.ticks,max:t.max?.value??d.max,min:t.min?.value??d.min,graticule:t.graticule?.value??d.graticule},g.options.ticks>32&&(s.c.warn(`Radar diagram ticks (${g.options.ticks}) exceeds maximum allowed (32). Using 32 instead.`),g.options.ticks=32)},"setOptions"),getConfig:x,clear:(0,o.e)(()=>{(0,l.ZH)(),g=structuredClone(u)},"clear"),setAccTitle:l.GN,getAccTitle:l.eu,setDiagramTitle:l.g2,getDiagramTitle:l.Kr,getAccDescription:l.Mx,setAccDescription:l.U$},b=(0,o.e)(e=>{(0,r.A)(e,w);let{axes:t,curves:a,options:i}=e;w.setAxes(t),w.setCurves(a),w.setOptions(i)},"populate"),C={parse:(0,o.e)(async e=>{let t=await (0,c.Qc)("radar",e);s.c.debug(t),b(t)},"parse")},M=(0,o.e)((e,t,a,r)=>{let n=r.db,l=n.getAxes(),s=n.getCurves(),o=n.getOptions(),c=n.getConfig(),d=n.getDiagramTitle(),u=k((0,i.P)(t),c),g=o.max??Math.max(...s.map(e=>Math.max(...e.entries))),p=o.min,x=Math.min(c.width,c.height)/2;L(u,l,x,o.ticks,o.graticule),T(u,l,x,c),A(u,l,s,p,g,o.graticule,c),S(u,s,o.showLegend,c),u.append("text").attr("class","radarTitle").text(d).attr("x",0).attr("y",-c.height/2-c.marginTop)},"draw"),k=(0,o.e)((e,t)=>{let a=t.width+t.marginLeft+t.marginRight,r=t.height+t.marginTop+t.marginBottom,i={x:t.marginLeft+t.width/2,y:t.marginTop+t.height/2};return(0,l.v2)(e,r,a,t.useMaxWidth??!0),e.attr("viewBox",`0 0 ${a} ${r}`).attr("overflow","visible"),e.append("g").attr("transform",`translate(${i.x}, ${i.y})`)},"drawFrame"),L=(0,o.e)((e,t,a,r,i)=>{if("circle"===i)for(let t=0;t<r;t++){let i=a*(t+1)/r;e.append("circle").attr("r",i).attr("class","radarGraticule")}else if("polygon"===i){let i=t.length;for(let n=0;n<r;n++){let l=a*(n+1)/r,s=t.map((e,t)=>{let a=2*t*Math.PI/i-Math.PI/2,r=l*Math.cos(a),n=l*Math.sin(a);return`${r},${n}`}).join(" ");e.append("polygon").attr("points",s).attr("class","radarGraticule")}}},"drawGraticule"),T=(0,o.e)((e,t,a,r)=>{let i=t.length;for(let n=0;n<i;n++){let l=t[n].label,s=2*n*Math.PI/i-Math.PI/2,o=Math.cos(s),c=Math.sin(s);e.append("line").attr("x1",0).attr("y1",0).attr("x2",a*r.axisScaleFactor*o).attr("y2",a*r.axisScaleFactor*c).attr("class","radarAxisLine");let d=o>.01?"start":o<-.01?"end":"middle",u=c>.01?"hanging":c<-.01?"auto":"central";e.append("text").text(l).attr("x",a*r.axisLabelFactor*o+4*o).attr("y",a*r.axisLabelFactor*c+4*c).attr("text-anchor",d).attr("dominant-baseline",u).attr("class","radarAxisLabel")}},"drawAxes");function A(e,t,a,r,i,n,l){let s=t.length,o=Math.min(l.width,l.height)/2;a.forEach((t,a)=>{if(t.entries.length!==s)return;let c=t.entries.map((e,t)=>{let a=2*Math.PI*t/s-Math.PI/2,n=E(e,r,i,o);return{x:n*Math.cos(a),y:n*Math.sin(a)}});"circle"===n?e.append("path").attr("d",O(c,l.curveTension)).attr("class",`radarCurve-${a}`):"polygon"===n&&e.append("polygon").attr("points",c.map(e=>`${e.x},${e.y}`).join(" ")).attr("class",`radarCurve-${a}`)})}function E(e,t,a,r){return r*(Math.min(Math.max(e,t),a)-t)/(a-t)}function O(e,t){let a=e.length,r=`M${e[0].x},${e[0].y}`;for(let i=0;i<a;i++){let n=e[(i-1+a)%a],l=e[i],s=e[(i+1)%a],o=e[(i+2)%a],c={x:l.x+(s.x-n.x)*t,y:l.y+(s.y-n.y)*t},d={x:s.x-(o.x-l.x)*t,y:s.y-(o.y-l.y)*t};r+=` C${c.x},${c.y} ${d.x},${d.y} ${s.x},${s.y}`}return`${r} Z`}function S(e,t,a,r){if(!a)return;let i=(r.width/2+r.marginRight)*3/4,n=-(3*(r.height/2+r.marginTop))/4;t.forEach((t,a)=>{let r=e.append("g").attr("transform",`translate(${i}, ${n+20*a})`);r.append("rect").attr("width",12).attr("height",12).attr("class",`radarLegendBox-${a}`),r.append("text").attr("x",16).attr("y",0).attr("class","radarLegendText").text(t.label)})}(0,o.e)(A,"drawCurves"),(0,o.e)(E,"relativeRadius"),(0,o.e)(O,"closedRoundCurve"),(0,o.e)(S,"drawLegend");var D=(0,o.e)((e,t)=>{let a="";for(let r=0;r<e.THEME_COLOR_LIMIT;r++){let i=e[`cScale${r}`];a+=`
		.radarCurve-${r} {
			color: ${i};
			fill: ${i};
			fill-opacity: ${t.curveOpacity};
			stroke: ${i};
			stroke-width: ${t.curveStrokeWidth};
		}
		.radarLegendBox-${r} {
			fill: ${i};
			fill-opacity: ${t.curveOpacity};
			stroke: ${i};
		}
		`}return a},"genIndexStyles"),R=(0,o.e)(e=>{let t=(0,l.xN)(),a=(0,l.iE)(),r=(0,n.Rb)(t,a.themeVariables),i=(0,n.Rb)(r.radar,e);return{themeVariables:r,radarOptions:i}},"buildRadarStyleOptions"),I={parser:C,db:w,renderer:{draw:M},styles:(0,o.e)(({radar:e}={})=>{let{themeVariables:t,radarOptions:a}=R(e);return`
	.radarTitle {
		font-size: ${t.fontSize};
		color: ${t.titleColor};
		dominant-baseline: hanging;
		text-anchor: middle;
	}
	.radarAxisLine {
		stroke: ${a.axisColor};
		stroke-width: ${a.axisStrokeWidth};
	}
	.radarAxisLabel {
		font-size: ${a.axisLabelFontSize}px;
		color: ${a.axisColor};
	}
	.radarGraticule {
		fill: ${a.graticuleColor};
		fill-opacity: ${a.graticuleOpacity};
		stroke: ${a.graticuleColor};
		stroke-width: ${a.graticuleStrokeWidth};
	}
	.radarLegendText {
		text-anchor: start;
		font-size: ${a.legendFontSize}px;
		dominant-baseline: hanging;
	}
	${D(t,a)}
	`},"styles")}}}]);